...
def create_test_file(
    ...
    :param num_pairs: number of pairs to include in the test file
    :param test_file_path: path to save the test file
    """
    featurizer = Featurizer(
        dataset_name=dataset_name, features=features, embeddings_dir=embeddings_dir
    )
    ...
    json_file = {
        "features": [
            f"{feature['operation']}({feature['field']})" for feature in features
        ],
        "examples": [],
    }
    ...
    for sig_id1, sig_id2, _ in train_pairs:
        if (
            sig_id1 in featurizer.extended_signatures
            and sig_id2 in featurizer.extended_signatures
        ):
            sig1 = featurizer.extended_signatures[sig_id1]
            sig2 = featurizer.extended_signatures[sig_id2]
            if embeddings_dir is not None:
                sig1["external_vector"] = featurizer.paper_ids_to_emb[
                    str(sig1["paper_id"])
                ]
                sig2["external_vector"] = featurizer.paper_ids_to_emb[
                    str(sig2["paper_id"])
                ]
            ...
            json_file["examples"].append(
                {
                    ...
                    "signature2": sig2,
                    "feature_vector": [
                        "NaN" if isnan(feature) else feature
